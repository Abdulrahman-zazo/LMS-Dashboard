...
export default function ComplaintDetailPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      <div className="flex-1 bg-black/30 backdrop-blur-xs" onClick={onClose} />
      <aside
        className={cn(
      ...
      >
        ...
        <div className="p-6 space-y-5 flex-1 overflow-y-auto">
          <p className="text-neutral-800 text-sm my-2 ">الشكوى أو الأقتراح:</p>

          <div className="rounded-lg  border p-4 text-neutral-800 text-sm leading-relaxed whitespace-pre-wrap">
            {complaint.text}
          </div>
        ...
        <div className="p-6 border-t space-y-6">
          ...
          >
            <Button className="w-full gap-2">
              <Mail size={16} /> الرد على الشكوى
            </Button>
          </a>
          <Button
            variant="ghost"
            onClick={() => onDelete(complaint.id)}
            className="w-full gap-2 mt-2 text-red-500/80"
          >
            <Trash2 size={16} /> حذف الشكوى
          </Button>
        </div>
      </aside>
    </div>
